...
function generateSchedule(viewName, renderStart, renderEnd) {
    ...
    for (var key in intervencoes) {
        var intervencao = intervencoes[key];
        var schedule = new ScheduleInfo();
        var calendar = CalendarList[parseInt(intervencao.situacao - 1)];
        schedule.id = intervencao.id;
        schedule.calendarId = String(intervencao.situacao);

        schedule.title = intervencao.assunto;
        schedule.body = intervencao.observacao;
        schedule.isReadOnly = false;
        schedule.isPrivate = false;
        schedule.location = String(intervencao.academia_id);
        schedule.attendees = [intervencao.usuario];
        schedule.recurrenceRule = 0;
        schedule.state = "Free";
        schedule.color = calendar.color;
        schedule.bgColor = calendar.bgColor;
        schedule.dragBgColor = calendar.dragBgColor;
