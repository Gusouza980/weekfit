...
class ProspeccoesController extends Controller
{
    ...
    public function quantidade_prospeccoes_diarias(){
        $quantidade_prospeccoes = Prospeccao::select(DB::raw('count(id) as quantidade'), DB::raw("DATE_FORMAT(created_at, '%Y-%m-%d') data"))->where("created_at", ">=", date("Y-m-d", strtotime("- 30 days")))->orderBy("data", "ASC")->groupBy('data')->get();
